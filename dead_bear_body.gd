...
## Dead bear body that can be harvested for pelt and meat
...
func _ready() -> void:
	...
	# Unique prompt name
	my_prompt_name = "BearHarvestPrompt_%s" % get_instance_id()
	...
	print("Dead bear body ready for harvesting")
...
func check_player_interaction(delta: float) -> void:
	...
	if not has_knife:
		...
		show_interaction_prompt("[E] Harvest Bear", true)
...
func show_interaction_prompt(text: String, can_interact: bool) -> void:
	...
	if not prompt:
		...
		progress_fill.color = Color(0.8, 0.2, 0.2)  # Red for harvesting
...
func harvest_bear() -> void:
	...
	print("Harvesting bear...")

	# Give items directly to player inventory instead of spawning
	...
	if inventory_ui and inventory_ui.inventory_manager:
		...
		# Give 1 pelt
		var pelt_resource = load("res://items/bear_pelt.tres")
		if pelt_resource:
			var pelt_item = pelt_resource.duplicate(true)
			pelt_item.current_stack = 1
			if not try_add_to_inventory(manager, pelt_item):
				spawn_item("res://pickup_bear_pelt.tscn")  # Drop if inventory full

		# Give 1-3 meat
		var meat_count = randi_range(1, 3)
		var meat_resource = load("res://items/bear_meat.tres")
		if meat_resource:
			...
			if not try_add_to_inventory(manager, meat_item):
				...
				for i in range(meat_count):
					spawn_item("res://pickup_bear_meat.tscn")

		inventory_ui.refresh_display()
		print("Harvested: 1 pelt, %d meat" % meat_count)
