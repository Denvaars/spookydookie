...
## Revolver/Magnum weapon system
## 6-shooter revolver with high damage and slower fire rate

# Revolver settings
@export var damage: float = 50.0  # High damage magnum rounds
@export var max_range: float = 50.0
@export var fire_rate: float = 0.7  # Slower fire rate (heavy trigger)
@export var reload_time: float = 2.0  # Slower reload (cylinder reload)
@export var max_ammo: int = 6  # 6-shooter
@export var aim_fov: float = 65.0
@export var normal_fov: float = 75.0
@export var spread_angle: float = 0.02  # Less spread, more accurate
...
var ammo_item_id: String = "pistol_ammo_01"
var ammo_per_stack: int = 6  # Matches magazine size
...
func _ready() -> void:
	...
	if player.equipped_weapon_item and player.equipped_weapon_item.weapon_current_ammo >= 0:
		...
		print("Restored pistol ammo: %d/%d" % [current_ammo, max_ammo])

	# Create visual model (larger box for revolver)
	mesh_instance = MeshInstance3D.new()
	var box_mesh = BoxMesh.new()
	box_mesh.size = Vector3(0.12, 0.18, 0.3)  # Larger revolver
	mesh_instance.mesh = box_mesh

	# Create material
	var material = StandardMaterial3D.new()
	material.albedo_color = Color(0.6, 0.6, 0.65)  # Metallic silver/steel
	...
	shoot_sound.stream = load("res://audio/revolver_shoot.wav")
...
func shoot() -> void:
	...
	current_ammo -= 1
	can_shoot = false
	print("Revolver shot! Ammo: ", current_ammo, "/", max_ammo)
	...
	if forest and "alert_system" in forest:
		...
		if alert_system:
			alert_system.alert_weapon_fire(player.global_position, "pistol")
...
func fire_bullet() -> void:
	...
	if result:
		...
		print("Revolver hit: ", hit_object.name, " at ", result.position)
...
func reload() -> void:
	...
	if ammo_in_inventory == 0:
		print("No pistol ammo in inventory!")
		return
...
func on_unequip() -> void:
	if player and player.equipped_weapon_item:
		...
		print("Saved pistol ammo: %d/%d" % [current_ammo, max_ammo])
